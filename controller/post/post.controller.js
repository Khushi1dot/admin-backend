...
class AdminPostController {
  ...
  static getById = catchAsync(async (req, res) => {
    ...
    if (!post) {
      ...
      // ✅ Safely access the first comment’s userId
  if (post.comments?.length > 0 && post.comments[0]?.userId) {
    console.log(post.comments[0].userId);
  } else {
    console.log("No comments or userId found");
  }
  ...
  static exportSinglePost = catchAsync(async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=post_${post._id}.xlsx`
      );
